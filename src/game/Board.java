...
public class Board {
    ...
    public void addRandomTile() {
        if (!openSpaces.isEmpty()) {
            int randomIndex = StdRandom.uniform(openSpaces.size()); // 1. Select a tile t by picking a random open space from openSpaces.
            BoardSpot randomTile = openSpaces.get(randomIndex);
            int randomValue;
            ...
            gameBoard[randomTile.getRow()][randomTile.getCol()] = randomValue;
        }
    }
            ...
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] == 0) {
                    for (int ptr = j + 1; ptr < 4; ptr++) {
                        if (gameBoard[i][ptr] != 0) {
                            gameBoard[i][j] = gameBoard[i][ptr];
                            gameBoard[i][ptr] = 0;
                            ptr += 4;
                        }
                    }
    ...
    public void mergeLeft() {
        for ( int i = 0 ; i < gameBoard.length ; i++ ) {
            for ( int j = 0; j < gameBoard[i].length; j++ ) {
                if ((j < gameBoard[i].length - 1) && (gameBoard[i][j] > 0) && (gameBoard[i][j+1] > 0)){
